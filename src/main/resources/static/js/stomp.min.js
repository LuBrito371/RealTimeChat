...
!function () {
  ...
  n = (function () {
    ...
    n.prototype.toString = function () {
      ...
      o = this.headers;
      for (n in o) {
        if (!r.call(o, n)) continue;
        i = o[n];
        e.push("" + n + ":" + i);
      }
      if (this.body) {
        e.push("content-length:" + ("" + this.body).length);
      }
  ...
  e = (function () {
    ...
    r.prototype._setupHeartbeat = function (n) {
      ...
      if (!(this.heartbeat.outgoing === 0 || r === 0)) {
        ...
        this.pinger = i.setInterval(s, function () {
          f.ws.send(t.LF);
          return typeof f.debug === "function" ? f.debug(">>> PING") : void 0;
        });
      }
      if (!(this.heartbeat.incoming === 0 || o === 0)) {
        ...
        return (this.ponger = i.setInterval(s, function () {
          var t;
          t = e() - f.serverActivity;
          if (t > s * 2) {
    ...
    r.prototype.connect = function () {
      ...
      this.ws.onmessage = function (i) {
        var r, o, u, a, f, h, l, p, d, g, b, m;
        a =
          typeof ArrayBuffer !== "undefined" && i.data instanceof ArrayBuffer
            ? ((r = new Uint8Array(i.data)),
              typeof c.debug === "function"
        ...
        for (d = 0, g = b.length; d < g; d++) {
          ...
          switch (f.command) {
            case "CONNECTED":
              if (typeof c.debug === "function") {
                c.debug("connected to server " + f.headers.server);
              }
              c.connected = true;
              c._setupHeartbeat(f.headers);
              m.push(
                typeof c.connectCallback === "function"
                  ? c.connectCallback(f)
                  : void 0
              );
              break;
            case "MESSAGE":
              ...
              l = c.subscriptions[p] || c.onreceive;
              if (l) {
                u = c;
                ...
                m.push(
                  typeof c.debug === "function"
                    ? c.debug("Unhandled received MESSAGE: " + f)
                    : void 0
                );
              }
              break;
            case "RECEIPT":
              m.push(
                typeof c.onreceipt === "function" ? c.onreceipt(f) : void 0
              );
              break;
            case "ERROR":
      ...
      this.ws.onclose = function () {
        var t;
        t = "Whoops! Lost connection to " + c.ws.url;
        if (typeof c.debug === "function") {
          c.debug(t);
        }
        c._cleanUp();
        return typeof s === "function" ? s(t) : void 0;
      };
      return (this.ws.onopen = function () {
        if (typeof c.debug === "function") {
          c.debug("Web Socket Opened...");
        }
        u["accept-version"] = i.VERSIONS.supportedVersions();
        u["heart-beat"] = [c.heartbeat.outgoing, c.heartbeat.incoming].join(
          ","
        );
        return c._transmit("CONNECT", u);
      });
    };
    r.prototype.disconnect = function (t) {
